...
public:
    ...
    void collectData()
    {
        if (origen.first>MAXX) MAXX=origen.first;
        if (origen.second>MAXY) MAXY=origen.second;
        if (longevity>MAXL) MAXL=longevity;
        //MAXL+=longevity;
    }

    void procesarCeldas(CellSpace& lasCeldas,int e_limit)
    {
        if (longevity<e_limit)
        {
...
public:
    ...
    void desordenar()
    {
        for (int k=0;k<10;k++) /*desordenar 10 veces*/
...
public:
    ...
    {
        sensors.clear();
        headers.clear();headers.push_back("T");
    ...
    void addData(QSpace quantaList,CellSpace cellList,int paso)
    {
        ...
        for (int s=0;s<sensors.size();s++)
            if (sensors[s].tipo=="SBI")
                ...
                {
                    int elSpot=sensors[s].datos[sp];
                    columna++;
                    cols[columna].push_back(quantaList.quanta[elSpot].origen.first);
                    columna++;
                    cols[columna].push_back(quantaList.quanta[elSpot].origen.second);
                    columna++;
                    cols[columna].push_back(quantaList.quanta[elSpot].valor);
                    columna++;
                    cols[columna].push_back(quantaList.quanta[elSpot].longevity);
                }
    }
    ...
    void outputHeaderSBR(ofstream& f)
    {   
        for (int i=0;i<sensors[0].datos.size();i++)
            f << sensors[0].datos[i] <<" "<<"-"<<" ";
        f << "\n";  
        for (int i=0;i<sensors[0].datos.size();i++)
            f << "Y" <<" "<<"L"<<" ";
    ...
    void outputRes(ofstream& f)
    {
        int totales[cols.size()];
        int filas=cols[0].size(); 
        for (int i=0;i<cols.size();i++) totales[i]=-1000000;
        for (int laFila=0;laFila<filas;laFila++)
            for (int c=0;c<cols.size();c++)
                if (cols[c][laFila]>totales[c]) totales[c]=cols[c][laFila];
        for (int c=0;c<cols.size();c++)
            f <<totales[c]<<" ";  
        f << "\n";     
    }

    void outputAll(ofstream& f,QSpace& spots,int pasos)
    {
        int inicio=0;
        int final;
        for (int i=0;i<sensors[0].datos.size();i++)
        {
            ...
            int totalY=-999999;
            int totalL=0;
            final=sensors[0].datos[i];
            for (int j=inicio;j<final;j++)
            {
                if (spots.quanta[j].MAXX>totalX) totalX=spots.quanta[j].MAXX; 
                if (spots.quanta[j].MAXY>totalY) totalY=spots.quanta[j].MAXY;
                totalL+=spots.quanta[j].MAXL;   
            }
            double promL=(double)totalL/double(final-inicio);
            f<<totalY<<" "<<promL<<" ";
            inicio=final;
        }
        f << "\n";     
...
public:
    ...
    void setElimit(int e_limit)
    {
        Spots.spotActivityLimit=e_limit;
    }
    ...
    bool abrir(string archivo)
    {
        ...
        char I;
        int x,y,n,s_index,limite;
        int regla,origx,origy,valor,r0,r1,spin,prior;
        vector<int> sensorList;
        string tipo_sensor;
        map<string,int> caso(
            {{"B",1},{"R",2},{"Q",3},{"E",4},{"S",5},{"L",6}}); 
        time_t sec;
        time(&sec); // Take the number of seconds and
        srand(getpid()+(unsigned)sec);
                ...
                switch(elcaso)
                {
                    ...
                    case 4:  /*"E":Block of random spots*/
                        ...
                        else  
                        {
                            ...
                            f >>regla>>I>>origx>>I>>origy>>I>>valor>>I>>r0>>I>>r1>>I>>spin>>I>>prior;
                            if (densidad<=1.0) /*cantidad aprox. determinada por la densidad*/
                            {
                                for (int i=0;i<nx;i++)
                                    for (int j=0;j<ny;j++)
                                    {
                            ...
                            else /*cantidad fija de spots=densidad, ubicación aleatoria*/
                                for (int k=0;k<int(densidad);k++)
                                {
                                    if (spin<0) elSpin=rand()%360; else elSpin=spin;
                                    int xx=origx+rand()%nx; int yy=origy+rand()%ny;
                                    putSpot(regla,xx,yy,valor,r0,r1,elSpin,prior);
                                }
                            desordenarSpots();        
                        }
                        break;
                    case 5:  /*"S":Sensors*/
                        ss >> I>>I>>tipo_sensor;
                        if (tipo_sensor=="SBI" or tipo_sensor=="SBR") /*Registra estado de spots individuales*/
                        {
                            f >> linea;
                            /*cout << linea << "\n";*/
    ...
    void doStep(int paso)
    {
        ...
        for (int i=0;i<Spots.quanta.size();i++) 
            Spots.quanta[i].procesarCeldas(Cells,Spots.spotActivityLimit);
        Cells.actualiza();
    }
    ...
    void runBat(string entrada, string salida, int pasos, int repetir)
    {
        ofstream o(salida);
        abrir(entrada);
        gauges.outputHeaderSBR(o);
        for(int rep=0;rep<repetir;rep++)
        {
            ...
            //gauges.outputRes(o);  
            //Spots.outputRes(o);
            gauges.outputAll(o,Spots,pasos);
            borrar();
            abrir(entrada);
        }
        o.close();
...
void displaySelectedSpots()
{
    glPolygonMode(GL_FRONT, GL_LINE);
    glPolygonMode(GL_BACK, GL_LINE);
    glBegin(GL_QUADS);
    for (int i=0;i<MiCA.Spots.quanta.size();i++)
        if (MiCA.Spots.quanta[i].priority !=0) plotSpot(MiCA.Spots.quanta[i]);
    glEnd();
}
...
void plotpoints()
{
    ...
    { 
        ...
        glEnd();
        if (showEnergy) displaySpots(); else displaySelectedSpots();
...
int main(int argc, char* argv[])
{
    ...
    if (argc>3) 
    {
        ...
        MiCA.runBat(f1,f2,timeOut,repeticiones);
    }
    else
        if (MiCA.abrir(f1))
        {
            cout << "Celdas:"<<" " <<MiCA.Cells.living.size() <<"\n";
